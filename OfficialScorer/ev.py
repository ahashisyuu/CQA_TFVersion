...
def read_res_pred_files(res_fname, pred_fname, attr, verbose=True,
                        reranking_th=0.0,
                        ignore_noanswer=False):
    global analyze_file, info_file
    lineReader = ResFileReader(attr)
    lineReader_pred = ResFileReader(attr)
    ...
    for line in irf:
        ...
        if len(ir[qid]) != 0:
            if ir[qid][-1] != 'NIL':  # Don't add any more results if we've already added the NIL value.
                # that's the candidate id, or NIL.
                # All candidates before the NIL are regarded as relevant.
                ir[qid].append(parts[0])
        else:
            ir[qid].append(parts[0])
        if parts[-1] == 'true':
            # collect the candidate ids of ALL true duplicates,
            # even the ones after the NIL, to be used in truncated MAP.
            q_rel[qid].append(parts[0])
...
def eval_reranker(res_fname="svm.test.res", pred_fname="svm.train.pred",
                  ...
                  reranking_th=0.0,
                  ignore_noanswer=False):
    ir, svm, conf_matrix = read_res_pred_files(res_fname, pred_fname, attr, verbose,
    ...
    print()
    print("REC-1 - percentage of questions with at least 1 correct answer "
          "in the top @X positions "
          "(useful for tasks where questions have at most one correct answer)")

    print("ACC   - accuracy, i.e., number of correct answers "
          "retrieved at rank @X normalized by the rank "
          "and the total number of questions")
    print("AC1   - the number of correct answers at @X "
          "normalized by the number of maximum possible answers (perfect re-ranker)")
    print("AC2   - the absolute number of correct answers at @X")


def eval_search_engine(res_fname, attr, th=10):
    ir = read_res_file(res_fname, attr)
    ...
    print()
    print("REC-1 - percentage of questions with at least 1 correct answer "
          "in the top @X positions (useful for tasks were questions have at most one correct answer)")
    print("ACC   - accuracy, i.e. number of correct answers "
          "retrieved at rank @X normalized by the rank and the total number of questions")
    print("AC1   - the number of correct answers at @X "
          "normalized by the number of maximum possible answers (perfect re-ranker)")
    print("AC2   - the absolute number of correct answers at @X")
