    ...
    def build_model(self):
        with tf.variable_scope('baseline', initializer=tf.glorot_uniform_initializer()):
            ...
            with tf.variable_scope('extract'):
                ...
                Q_vec = rnn(Q, seq_len=self.QS_len, return_type=0)
                C_vec = rnn(self.CT, seq_len=self.CT_len, return_type=0)

            info = tf.concat([Q_vec, C_vec, Q_vec - C_vec, Q_vec * C_vec], axis=1)
